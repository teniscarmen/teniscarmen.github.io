...
let iconClass = '';
switch(type) {
case 'success': iconClass = 'fa-check-circle text-green-500'; break;
case 'error': iconClass = 'fa-times-circle text-red-500'; break;
case 'warning': iconClass = 'fa-exclamation-triangle text-yellow-500'; break;
default: iconClass = 'fa-info-circle text-blue-500';
}
...
`;
document.getElementById('alertConfirmBtn').onclick = () => { onConfirm(); hideModal(alertModal); };
document.getElementById('alertCancelBtn').onclick = () => hideModal(alertModal);
} else {
...
document.getElementById('alertOkBtn').onclick = () => hideModal(alertModal);
}
...
periodSummaryDiv.innerHTML = '<p class="col-span-full text-center text-gray-500">Calculando...</p>';
globalSummaryDiv.innerHTML = '<p class="text-center text-gray-500">Calculando...</p>';
...
totalVentasPeriodo = ventasEnPeriodo.reduce((acc, v) => acc + (v.precioPactado || 0), 0);
...
totalAbonosPeriodo = abonosEnPeriodo.reduce((acc, a) => acc + (a.monto || 0), 0);

totalPendienteGlobal = localVentas.reduce((acc, v) => acc + (v.saldo || 0), 0);
...
`;
document.getElementById('corteXBtn').addEventListener('click', handleCorteX);
}
...
`;
document.getElementById('corteZBtn').addEventListener('click', handleCorteZ);
}
}
...
const abonosPendientes = allAbonos.filter(a => a.estado === 'pendiente' && a.enPosesionDe === currentUser.displayName);

if (abonosPendientes.length === 0) {
showAlert('Sin pendientes', 'No tienes dinero pendiente para transferir.', 'info');
return;
}
...
batch.update(abonoRef, { estado: 'entregado_a_carmen', corteId: corteRef.id });
});
...
showAlert('Error', 'Ocurrió un problema al registrar el corte.', 'error');
}
});
}

async function handleCorteZ() {
const abonosParaCorte = allAbonos.filter(a => a.estado === 'entregado_a_carmen');

if (abonosParaCorte.length === 0) {
showAlert('Sin dinero para cortar', 'No hay dinero pendiente de corte final.', 'info');
return;
}
...
const totalComision = Object.values(comisionesPorVendedor).reduce((s, c) => s + c, 0);

if (totalComision > 0) {
comisionHtml = `
<div class="text-left text-sm mt-4 p-2 bg-gray-100 rounded">
<h4 class="font-bold">Comisiones Generadas:</h4>
${Object.entries(comisionesPorVendedor).map(([vendedor, monto]) => `
...
abonosParaCorte.forEach(abono => {
const abonoRef = doc(db, getSharedCollectionPath('abonos'), abono.id);
batch.update(abonoRef, { estado: 'entregado_a_jr', corteId: corteRef.id });
});
...
try {
await batch.commit();
showAlert('Corte Z Realizado', 'El corte final fue registrado y las comisiones calculadas.', 'success');
} catch (error) {
console.error("Error en corte final:", error);
showAlert('Error', 'Ocurrió un problema al registrar el corte final.', 'error');
}
}
);
setTimeout(() => document.getElementById('alertMessage').insertAdjacentHTML('afterend', comisionHtml), 50);
}
...
showAlert('Confirmar Reversión de Corte', '¿Estás seguro? Esto eliminará el corte y revertirá los abonos y comisiones asociadas.', 'warning', async () => {
const batch = writeBatch(db);
...
if (corteType === 'Corte Z' && corte.comisionesPorVendedor && Object.keys(corte.comisionesPorVendedor).length > 0) {
const ventasLiquidadasEnCorte = localVentas.filter(v => v.comisionPagada && corte.abonosIds.some(aid => allAbonos.find(a => a.id === aid)?.ventaId === v.id));
...
const searchTerm = document.getElementById('searchInventario').value.toUpperCase().trim();
const brand = document.getElementById('filterMarca').value;
const gender = document.getElementById('filterGenero').value;
const size = document.getElementById('filterTalla').value.trim().toUpperCase();
...
if (gender) {
filteredInventario = filteredInventario.filter(i => i.genero === gender);
}
if (size) {
filteredInventario = filteredInventario.filter(i => i.talla.toUpperCase().includes(size));
}

switch(sortMethod) {
case 'precio_asc': filteredInventario.sort((a, b) => a.precio - b.precio); break;
case 'precio_desc': filteredInventario.sort((a, b) => b.precio - a.precio); break;
case 'marca_asc': filteredInventario.sort((a, b) => a.marca.localeCompare(b.marca)); break;
case 'marca_desc': filteredInventario.sort((a, b) => b.marca.localeCompare(a.marca)); break;
case 'reciente': default:
filteredInventario.sort((a, b) => (b.fechaRegistro?.seconds || 0) - (a.fechaRegistro?.seconds || 0));
}
...
const statusColor = item.status === 'disponible' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800';
const statusText = item.status === 'disponible' ? 'Disponible' : 'Vendido';
const card = document.createElement('div');
card.className = 'bg-white rounded-xl shadow-sm border border-gray-200 p-4 flex flex-col sm:flex-row gap-4 items-start';
...
document.querySelectorAll('.editInventarioBtn').forEach(btn => btn.addEventListener('click', handleEditInventario));
document.querySelectorAll('.deleteInventarioBtn').forEach(btn => btn.addEventListener('click', handleDeleteInventario));
};
...
const card = document.createElement('div');
card.className = 'bg-white rounded-xl shadow-sm border border-gray-200 p-4 flex flex-col justify-between text-center';
let obsHtml = '';
if(c.observaciones) {
...
document.querySelectorAll('.generateReportBtn').forEach(btn => btn.addEventListener('click', handleGenerateReport));
document.querySelectorAll('.abonoCuentaBtn').forEach(btn => btn.addEventListener('click', handleOpenAbonoCuenta));
document.querySelectorAll('.clientHistoryBtn').forEach(btn => btn.addEventListener('click', handleShowClientHistory));
document.querySelectorAll('.editClienteBtn').forEach(btn => btn.addEventListener('click', handleEditCliente));
document.querySelectorAll('.deleteClienteBtn').forEach(btn => btn.addEventListener('click', handleDeleteCliente));
document.querySelectorAll('.gemini-btn').forEach(btn => btn.addEventListener('click', handleOpenCobranzaModal));
};
...
const searchTerm = document.getElementById('searchVentas').value.toUpperCase().trim();
...
if(vendedor) filteredVentas = filteredVentas.filter(v => v.vendedor === vendedor);
if(estado === 'con_saldo') filteredVentas = filteredVentas.filter(v => v.saldo > 0);
else if (estado === 'liquidado') filteredVentas = filteredVentas.filter(v => v.saldo <= 0);

switch(sortMethod) {
case 'reciente': filteredVentas.sort((a,b) => b.fecha.seconds - a.fecha.seconds); break;
case 'antigua': filteredVentas.sort((a,b) => a.fecha.seconds - b.fecha.seconds); break;
case 'saldo_desc': filteredVentas.sort((a,b) => b.saldo - a.saldo); break;
case 'saldo_asc': filteredVentas.sort((a,b) => a.saldo - b.saldo); break;
case 'cliente_az': filteredVentas.sort((a,b) => (allClientes[a.clienteId]?.nombre || '').localeCompare(allClientes[b.clienteId]?.nombre || '')); break;
case 'cliente_za': filteredVentas.sort((a,b) => (allClientes[b.clienteId]?.nombre || '').localeCompare(allClientes[a.clienteId]?.nombre || '')); break;
}

const totalVendidoFiltrado = filteredVentas.reduce((sum, v) => sum + (v.precioPactado || 0), 0);
const saldoPendienteFiltrado = filteredVentas.reduce((sum, v) => sum + (v.saldo || 0), 0);
header.innerHTML = `
...
<div class="w-full sm:w-auto flex-shrink-0 flex sm:flex-col justify-end gap-2 pt-2 sm:pt-0 sm:border-l sm:pl-4">
<button class="addAbonoBtn text-sm text-white font-bold py-2 px-3 rounded-lg flex items-center justify-center ${v.saldo <= 0 ? 'bg-gray-400 cursor-not-allowed' : 'bg-emerald-500 hover:bg-emerald-600'}" data-id="${v.id}" ${v.saldo <= 0 ? 'disabled' : ''} title="Abonar"><i class="fas fa-hand-holding-dollar fa-lg"></i></button>
...
document.querySelectorAll('.addAbonoBtn').forEach(btn => btn.addEventListener('click', handleAddAbono));
document.querySelectorAll('.deleteVentaBtn').forEach(btn => btn.addEventListener('click', handleDeleteVenta));
document.querySelectorAll('.editVentaBtn').forEach(btn => btn.addEventListener('click', handleEditVenta));
};
...
const sortedCortes = cortes.sort((a, b) => b.fecha.seconds - a.fecha.seconds);
...
document.querySelectorAll('#cortes-x-history-list .deleteCorteBtn').forEach(btn => {
btn.addEventListener('click', (e) => handleDeleteCorte(e.currentTarget.dataset.id, e.currentTarget.dataset.type));
});
}
...
document.querySelectorAll('#cortes-z-history-list .deleteCorteBtn').forEach(btn => {
btn.addEventListener('click', (e) => handleDeleteCorte(e.currentTarget.dataset.id, e.currentTarget.dataset.type));
});
}
...
const vendedores = [...new Set(ventas.map(v => v.vendedor).filter(v => v))].sort();
...
document.getElementById('inventarioModalTitle').textContent = 'Editar Tenis';
...
showAlert('Acción no permitida', 'No se puede eliminar un tenis que ya ha sido vendido. Primero debe eliminar la venta asociada.', 'error');
return;
}
...
'warning',
async () => {
const q = query(collection(db, getSharedCollectionPath('ventas')), where("clienteId", "==", id));
const querySnapshot = await getDocs(q);
if (!querySnapshot.empty) {
showAlert('Error', 'No se puede eliminar. El cliente tiene ventas registradas.', 'error');
return;
}
try {
...
const sales = ventasCliente.sort((a,b) => b.fecha.seconds - a.fecha.seconds);
...
const abonosQuery = query(collection(db, getSharedCollectionPath('abonos')), where("ventaId", "==", id));
const abonosSnapshot = await getDocs(abonosQuery);
abonosSnapshot.forEach(doc => batch.delete(doc.ref));

batch.delete(ventaRef);

const inventarioRef = doc(db, getSharedCollectionPath('inventario'), ventaData.tenisId);
batch.update(inventarioRef, { status: "disponible" });

await batch.commit();
showAlert('Éxito', 'La venta ha sido eliminada y el tenis ha sido restaurado al inventario.', 'success');
} catch (error) {
...
const abonosQuery = query(collection(db, getSharedCollectionPath('abonos')), where("ventaId", "==", ventaId));
onSnapshot(abonosQuery, (snapshot) => {
abonosList.innerHTML = '';
if (snapshot.empty) {
abonosList.innerHTML = '<li class="text-gray-400">No hay abonos previos.</li>';
} else {
const abonos = snapshot.docs.map(doc => ({id: doc.id, ...doc.data()}));
abonos.sort((a,b) => a.fecha.seconds - b.fecha.seconds);

abonos.forEach(abono => {
const metodoClass = abono.metodoPago === 'Efectivo' ? 'bg-green-100 text-green-800' : 'bg-blue-100 text-blue-800';
...
document.querySelectorAll('.editAbonoBtn').forEach(btn => btn.addEventListener('click', handleEditAbono));
document.querySelectorAll('.deleteAbonoBtn').forEach(btn => btn.addEventListener('click', handleDeleteAbono));
}
});
...
document.getElementById('editAbonoFecha').value = fecha.toISOString().split('T')[0];
...
const saldoTotal = ventasCliente.reduce((sum, venta) => sum + venta.saldo, 0);
...
const ventasCliente = localVentas.filter(v => v.clienteId === clienteId && v.saldo > 0);
if (ventasCliente.length === 0) {
showAlert('Sin Deudas', 'Este cliente no tiene saldos pendientes.', 'info');
return;
}
...
async function generateCobranzaMessage(cliente, ventas, tono = "amable y profesional") {
if (geminiApiKey === "TU_API_KEY_AQUÍ" || !geminiApiKey) {
showAlert('Clave de API Faltante', 'Por favor, añade tu clave de API de Gemini para usar esta función.', 'warning');
return;
}
...
const detallesArticulos = ventas.map(v => {
const tenis = allInventario[v.tenisId];
...
messageTextarea.value = "No se pudo generar el mensaje. Por favor, inténtalo de nuevo.";
}
} catch (error) {
...
if (!marca || !modelo) {
showAlert('Datos insuficientes', 'Por favor, introduce al menos la marca y el modelo para generar una descripción.', 'warning');
return;
}
...
const ventasCliente = localVentas.filter(v => v.clienteId === clienteId).sort((a,b) => b.fecha.seconds - a.fecha.seconds);
const abonosCliente = allAbonos.filter(a => ventasCliente.some(v => v.id === a.ventaId));
...
const abonosVenta = abonosCliente.filter(a => a.ventaId === venta.id).sort((a,b) => a.fecha.seconds - b.fecha.seconds);
let abonosHtml = '<tr><td colspan="4" style="text-align: center; color: #6b7280; padding: 8px;">No se han registrado abonos para esta compra.</td></tr>';

if (abonosVenta.length > 0) {
abonosHtml = abonosVenta.map(abono => `
<tr>
...
if (!button) return;
document.getElementById('tabs').querySelectorAll('.tab-button').forEach(btn => btn.classList.remove('active'));
button.classList.add('active');
document.getElementById('tab-content').querySelectorAll('div[id$="-content"]').forEach(content => content.classList.add('hidden'));
...
// Filter listeners
['filterMarca', 'filterGenero', 'filterTalla', 'sortInventario', 'searchInventario'].forEach(id => { document.getElementById(id).addEventListener('input', renderInventario); });
...
unsubscribeListeners.push(unsubClientes, unsubInventario, unsubVentas, unsubAbonos, unsubCortes);
}
...
showAlert('Error Crítico', 'No se pudo inicializar la sesión de Firebase.', 'error');
}
});
